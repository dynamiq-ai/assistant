import { ChatMessage, ChatWidgetOptions, ApiConfig } from "./types";
import "./styles.css";
...
export class ChatWidgetCore {
  ...
  constructor(container: HTMLElement, options: ChatWidgetOptions = {}) {
    ...
    this.userId = options.api?.userId || crypto.randomUUID();
    this.sessionId = options.api?.sessionId || crypto.randomUUID();
    ...
    try {
      ...
      if (this.apiConfig.streaming) {
        ...
        while (true) {
          ...
          // Each chunk might contain multiple `data:` events, so split by line
          const events = value.trim().split("\n");

          events.forEach((eventLine) => {
            if (eventLine.startsWith("data:")) {
              const jsonData = eventLine.slice(5).trim(); // Remove 'data:' prefix
